...
void sr_nat_mapping_destroy(struct sr_nat *nat, struct sr_nat_mapping *mapping){
	...
	struct sr_unsolicited_pkts *unsolicited, *nxt_unsolicited;
        
	for (unsolicited = nat->unsolicited_pkts; unsolicited; unsolicited = nxt_unsolicited) {
		nxt_unsolicited = unsolicited->next;
		free(unsolicited);
	}
		...
		if(pkt_dir == incoming){
			...
			if(mapping == NULL){
			  if(tcp_hdr->tcp_flag & SYN){	
				...
				if(unsolicited == NULL){
					printf("empty que\n");
					struct sr_unsolicited_pkts *new_unsolicited = (struct sr_unsolicited_pkts *)malloc(sizeof(struct sr_unsolicited_pkts));
					new_unsolicited->packet=(uint8_t *) malloc(len);
					memcpy(new_unsolicited->packet, packet, len);
